...
var campaignIds = {
    'default': "65c4e59c3593a500010dea46",
}
var cookieDomain = "my-quotehealth.com"
...
var initialSrc = "https://track.myquotehealth.online/" + campaignID + "?format=json" + "&referrer=" + referrer;
for (var i = 1; i <= 10; i++) {
...
if (!urlParams.get('rtkcid')) {
    ...
    rtkxhr.onreadystatechange = function () {
        if (rtkxhr.readyState == 4 && rtkxhr.status == 200) {
            ...
            if (sessionStorage.getItem('viewOnce') !== 1) {
                xhrr.open("GET", "https://track.myquotehealth.online/view?clickid=" + rawData.clickid + "&referrer=" + referrer)
                xhrr.send();
            }
    ...
    if (sessionStorage.getItem('rtkclickid') === 'undefined' || !sessionStorage.getItem('rtkclickid')) {
        ...
        xhrr = new XMLHttpRequest;
        if (sessionStorage.getItem('viewOnce') !== 1) {
            xhrr.open("GET", "https://track.myquotehealth.online/view?clickid=" + rtkClickID + "&referrer=" + referrer)
            xhrr.send();
        }
        if (registerViewOncePerSession) {
            sessionStorage.setItem('viewOnce', '1')
    ...
    if (sessionStorage.getItem('viewOnce') !== 1) {
        xhrTrack.open("GET", "https://track.myquotehealth.online/view?clickid=" + rtkClickID + "&referrer=" + referrer)
        xhrTrack.send();
    }
    if (registerViewOncePerSession) {
        sessionStorage.setItem('viewOnce', '1')
